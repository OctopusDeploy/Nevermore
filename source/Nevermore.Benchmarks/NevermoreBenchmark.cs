...
namespace Nevermore.Benchmarks
{
    ...
    public class NevermoreBenchmark : BenchmarkBase
    {
        ...
        IReadTransaction nevermoreTransaction;
        ...
        public override void SetUp()
        {
            ...
            nevermoreTransaction = store.BeginReadTransaction();
        }

        [Benchmark(Description = "List 100 posts")]
        public List<Post> List100Posts()
        {
            var results = nevermoreTransaction.Stream<Post>("select Top 100 * from Posts").ToList();
        ...
        {
            var results = nevermoreTransaction.Stream<(string Id, long PostLength)>("select Top 50 Id, Len([Text]) as PostLength from Posts").ToList();
        ...
        {
            var results = nevermoreTransaction.Stream<long?>("select top 100 Len([Text]) as PostLength from Posts").ToList();
        ...
        {
            var results = nevermoreTransaction.Stream<Customer>("select top 100 * from dbo.Customer").ToList();
        ...
        {
            var results = nevermoreTransaction.Query<Customer>().Take(100).ToList();
